...
class Solution {
    ...
    public long multiplyTwoLists(Node first, Node second) {
        ...
        // Traverse the first linked list and generate the number
        while (first != null) {
            num1 = (num1 * 10 + first.data) % MOD;
            first = first.next;
        }

        // Traverse the second linked list and generate the number
        while (second != null) {
            num2 = (num2 * 10 + second.data) % MOD;
            second = second.next;
        }

        // Return the result of multiplication under modulo
